...
import minitype.types.ArrayType;
import minitype.types.BaseType;
import minitype.types.ErrorType;
import minitype.types.FunctionType;
import minitype.types.PointerType;
 ...
 * <p>
 * It extends the ANTLR-generated {@code MiniTypeBaseVisitor<Type>}.  Each {@code visitX} method corresponds to ONE
 * grammar production and is the semantic action that SYNTHESIZES that node's type from its children's types — exactly
 * the course's {@code { ... }} rules that compute the {@code .type} attribute.  On a mismatch it records an error via
 * {@link Diagnostics} and returns {@link ErrorType} ({@code type_error}), which propagates upward WITHOUT cascading
 * (each rule first checks {@code isError()} on its operands).
 * <p>
 * The program is checked in two passes (see {@link #visitProgram}) so that declarations may be used before the point
 * where they textually appear (mutual recursion, forward references, recursive structs).
 */
public final class TypeChecker extends MiniTypeBaseVisitor<Type> {
    ...
    public TypeChecker(Diagnostics diag) {
        this.diag = diag;
    }
    ...
    public Type visitProgram(MiniTypeParser.ProgramContext ctx) {
        ...
        for (MiniTypeParser.TopDeclContext td : ctx.topDecl()) {
            if (td.structDecl() != null) {
                declareStructFields(td.structDecl());
            }
        }
        ...
        for (MiniTypeParser.TopDeclContext td : ctx.topDecl()) {
            if (td.funcDecl() != null) {
                declareFunction(td.funcDecl());
            } else if (td.varDecl() != null) {
                declareVarType(td.varDecl());
            }
        }
        // Pass 2 — check function bodies and global initialisers.
        for (MiniTypeParser.TopDeclContext td : ctx.topDecl()) {
            if (td.funcDecl() != null) {
                checkFunctionBody(td.funcDecl());
            } else if (td.varDecl() != null) {
                checkInit(sym.lookup(td.varDecl().ID().getText()), td.varDecl());
            }
        }
        return VOID;
    }
    ...
    private Type resolveType(MiniTypeParser.TypeContext ctx) {
        ...
        switch (base) {
            ...
            default: // a struct name
                ...
                if (r == null) {
                    diag.error(ctx.getStart(), "unknown type '" + base + "'");
                    t = ERR;
                } else {
                    t = r;
                }
        ...
        for (int i = 0; i < ctx.star().size(); i++) {
            t = new PointerType(t); // each '*' wraps in Pointer
        }
        return t;
    }
    ...
    private void declareStructFields(MiniTypeParser.StructDeclContext sd) {
        ...
        if (rec == null) {
            return; // was a redefinition; skip
        }
        for (MiniTypeParser.FieldDeclContext f : sd.fieldDecl()) {
            ...
            if (ft == VOID) {
                diag.error(f.getStart(), "field '" + f.ID().getText() + "' cannot have type void");
                ft = ERR;
            }
            if (f.arraySuffix() != null) {
                ft = new ArrayType(ft);
            }
    ...
    private void declareFunction(MiniTypeParser.FuncDeclContext fd) {
        ...
        if (fd.paramList() != null) {
            for (MiniTypeParser.ParamContext p : fd.paramList().param()) {
                ...
                if (pt == VOID) {
                    diag.error(p.getStart(), "parameter '" + p.ID().getText() + "' cannot have type void");
                    pt = ERR;
                }
                if (p.arraySuffix() != null) {
                    pt = new ArrayType(pt);
                }
                params.add(pt);
            }
    ...
    private Type declareVarType(MiniTypeParser.VarDeclContext vd) {
        ...
        if (t == VOID) {
            diag.error(vd.getStart(), "variable '" + vd.ID().getText() + "' cannot have type void");
            t = ERR;
        }
        if (vd.arraySuffix() != null) {
            t = new ArrayType(t);
        }
    ...
    private void checkInit(Type declared, MiniTypeParser.VarDeclContext vd) {
        ...
        if (!declared.isError() && !init.isError() && !declared.same(init)) {
            diag.error(vd.expr().getStart(),
                       "cannot initialize '" + vd.ID().getText() + "' of type " + declared + " with value of type "
                           + init);
        }
    }
    ...
    @Override
    public Type visitDeclStmt(MiniTypeParser.DeclStmtContext ctx) {
    ...
    @Override
    public Type visitBlockStmt(MiniTypeParser.BlockStmtContext ctx) {
        return visit(ctx.block());
    }

    @Override
    public Type visitBlock(MiniTypeParser.BlockContext ctx) {
        sym.enterScope();
        for (MiniTypeParser.StmtContext s : ctx.stmt()) {
            visit(s);
        }
        sym.exitScope();
        return VOID;
    ...
    @Override
    public Type visitIfStmt(MiniTypeParser.IfStmtContext ctx) {
        Type c = visit(ctx.expr());
        if (!c.isError() && !c.same(BOOL)) {
            diag.error(ctx.expr().getStart(), "if condition must be bool, got " + c);
        }
        for (MiniTypeParser.StmtContext s : ctx.stmt()) {
            visit(s); // then + optional else
        }
        return VOID;
    }

    @Override
    public Type visitWhileStmt(MiniTypeParser.WhileStmtContext ctx) {
        Type c = visit(ctx.expr());
        if (!c.isError() && !c.same(BOOL)) {
    ...
    @Override
    public Type visitReturnStmt(MiniTypeParser.ReturnStmtContext ctx) {
    ...
    @Override
    public Type visitAssignStmt(MiniTypeParser.AssignStmtContext ctx) {
    ...
    @Override
    public Type visitExprStmt(MiniTypeParser.ExprStmtContext ctx) {
        visit(ctx.expr());
        return VOID;
    }

    /**
     * An expression is assignable / addressable iff it is a variable, an array element, a struct field, or a pointer
     * dereference.
     */
    private boolean isLvalue(MiniTypeParser.ExprContext e) {
        if (e instanceof MiniTypeParser.IdentContext) {
            return true;
        }
        if (e instanceof MiniTypeParser.IndexContext) {
            return true;
        }
        if (e instanceof MiniTypeParser.FieldContext) {
            return true;
        }
    ...
    @Override
    public Type visitIntLit(MiniTypeParser.IntLitContext ctx) {
        return INT;
    }

    @Override
    public Type visitDoubleLit(MiniTypeParser.DoubleLitContext ctx) {
        return DOUBLE;
    }

    @Override
    public Type visitCharLit(MiniTypeParser.CharLitContext ctx) {
        return CHAR;
    }

    @Override
    public Type visitBoolLit(MiniTypeParser.BoolLitContext ctx) {
        return BOOL;
    }

    @Override
    public Type visitParen(MiniTypeParser.ParenContext ctx) {
        return visit(ctx.expr());
    }

    @Override
    public Type visitIdent(MiniTypeParser.IdentContext ctx) {
        String name = ctx.ID().getText();
        Type t = sym.lookup(name);
        if (t == null) {
            return fail(ctx, "undefined name '" + name + "'");
        }
        return t;
    }

    @Override
    public Type visitCast(MiniTypeParser.CastContext ctx) {
        Type target = baseTypeOf(ctx.baseType());
        Type t = visit(ctx.expr());
        if (t.isError()) {
            return ERR;
        }
        if (allowConversion(t, target)) {
            return target;
        }
    ...
    @Override
    public Type visitUnary(MiniTypeParser.UnaryContext ctx) {
        ...
        switch (op) {
            ...
            case "*": // dereference
                if (t instanceof PointerType) {
                    return ((PointerType) t).pointee;
                }
                return fail(ctx, "cannot dereference non-pointer type " + t);
            case "&": // address-of
                if (!isLvalue(ctx.expr())) {
                    return fail(ctx, "cannot take the address of a non-lvalue");
                }
                if (t instanceof ArrayType) {
                    return fail(ctx, "cannot take the address of an array (" + t + ")");
                }
                if (t instanceof FunctionType) {
                    return fail(ctx, "cannot take the address of a function");
                }
    ...
    @Override
    public Type visitMulDiv(MiniTypeParser.MulDivContext ctx) {
        String op = ctx.op.getText();
    ...
    @Override
    public Type visitRel(MiniTypeParser.RelContext ctx) {
        String op = ctx.op.getText();
        Type l = visit(ctx.expr(0)), r = visit(ctx.expr(1));
        if (l.isError() || r.isError()) {
            return ERR;
        }
        if (l.same(r) && (l.same(INT) || l.same(DOUBLE) || l.same(CHAR))) {
            return BOOL;
        }
        return fail(ctx,
                    "operator '" + op + "' requires matching int, double or char operands, got " + l + " and " + r);
    }

    @Override
    public Type visitEq(MiniTypeParser.EqContext ctx) {
        String op = ctx.op.getText();
        Type l = visit(ctx.expr(0)), r = visit(ctx.expr(1));
        if (l.isError() || r.isError()) {
            return ERR;
    ...
    @Override
    public Type visitAnd(MiniTypeParser.AndContext ctx) {
        return logical("&&", ctx, ctx.expr(0), ctx.expr(1));
    }

    @Override
    public Type visitOr(MiniTypeParser.OrContext ctx) {
        return logical("||", ctx, ctx.expr(0), ctx.expr(1));
    }
    ...
    @Override
    public Type visitIndex(MiniTypeParser.IndexContext ctx) {
        Type arr = visit(ctx.expr(0)), idx = visit(ctx.expr(1));
        if (arr.isError() || idx.isError()) {
            return ERR;
        }
        if (!(arr instanceof ArrayType)) {
            return fail(ctx, "cannot index non-array type " + arr);
        }
        if (!idx.same(INT)) {
            return fail(ctx, "array index must be int, got " + idx);
        }
        return ((ArrayType) arr).elem;
    }

    @Override
    public Type visitField(MiniTypeParser.FieldContext ctx) {
        ...
        if (!(rec instanceof RecordType)) {
            return fail(ctx, "field access '." + field + "' on non-struct type " + rec);
        }
        RecordType r = (RecordType) rec;
        Type ft = r.fields.get(field);
        if (ft == null) {
            return fail(ctx, "struct " + r.name + " has no field '" + field + "'");
        }
        return ft;
    }

    @Override
    public Type visitCall(MiniTypeParser.CallContext ctx) {
        ...
        for (MiniTypeParser.ExprContext a : argExprs) {
        ...
        if (!(callee instanceof FunctionType)) {
            return fail(ctx, "cannot call non-function type " + callee);
        }
     ...
     */
    private boolean allowConversion(Type from, Type to) {
        return isScalar(from) && isScalar(to);
    }

    private boolean isScalar(Type t) {
        return t.same(INT) || t.same(DOUBLE) || t.same(CHAR);
    }
}

 ...
 * <p>
 * A stack of scopes models the global scope, each function-body scope, and any nested block scope.  {@link #define} =
 * addType (into the current scope), {@link #lookup} = getType (searches inner-to-outer).  Named struct (record) types
 * live in a separate global table since structs are always top-level.
 */
public final class SymbolTable {
    ...
    /**
     * addType(id, type): define {@code name} in the CURRENT (innermost) scope. Returns false if the name is already
     * defined in that same scope (i.e. a redefinition); shadowing an outer scope is allowed.
     */
    ...
    public void defineStruct(RecordType r) {
        structs.put(r.name, r);
    }

    public RecordType lookupStruct(String name) {

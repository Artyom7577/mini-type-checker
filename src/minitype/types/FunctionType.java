...
/**
 * Function type  T1 x T2 x ... x Tn -> R.
 * Course rule:  D -> T id ( L ) ;   =>   addType(id, L.type -> T.type),
 * where the parameter list L.type is the cartesian product of the parameter
 * types.  Printed as "(T1, T2) -> R" (or "T1 -> R" for a single parameter).
 * Structural equality: same arity, pairwise-equal params, equal return type.
 */
public final class FunctionType extends Type {
    ...
    @Override public boolean same(Type other) {
        ...
        for (int i = 0; i < params.size(); i++)
            if (!params.get(i).same(f.params.get(i))) return false;
